...
class ahkrun(sublime_plugin.WindowCommand):
	def run(self):
		...
		if not os.path.isfile(AutoHotKeyExePath):
			AutoHotKeyExePath = sublime.load_settings("AutoHotkey.sublime-settings").get("AutoHotKeyExePath")[0]
...
class ahkcompile(sublime_plugin.WindowCommand):
	def run(self):
		...
		if not os.path.isfile(Ahk2ExePath):
			Ahk2ExePath = sublime.load_settings("AutoHotkey.sublime-settings").get("Ahk2ExePath")[0]
...
class ahkrunpipedCommand(ahkrunpiped):
	...
	def run(self, edit, version='default'):
		...
		if not os.path.isfile(AutoHotKeyExePath):
			self.ahkpath = sublime.load_settings("AutoHotkey.sublime-settings").get("AutoHotKeyExePath")[0]
